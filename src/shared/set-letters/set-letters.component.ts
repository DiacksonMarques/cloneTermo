...
export class SetLettersComponent implements AfterViewInit, OnChanges, OnInit {
  ...
  addLetter(event: KeyboardEvent | string, index: number): void{
    ...
    if(typeof event === 'string'){
      if(index >= 0){
        ...
        } else if(this.checkNextEmptyLetter(nextIndex) < 6) {
  ...
  private bacskPaceLetter(index: number): void {
    if(this.currentWord){
      if(this.checkNextEmptyLetter(index) === 6 && this.indexSelect === -1){
        this.letters[5].letter = '';
        this.letters[5].select = true;
        this.indexSelect = 5;
        this.focusInput(5);
    ...
    if(this.currentWord){
      ...
      for (let index = 0; index < 6; index++) {
        if(this.letters[index].letter === ''){
          this.emitMessage.emit({severity:'info', summary:'Informação', detail:'Só palavras com 6 letras'});
          return;
        } else {
